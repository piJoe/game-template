      ...
      <ul class="answers">
        ${a.map((d,s)=>`<li data-answer="${s}">
          ...
          <div class="list-row-entry setting-row-entry">
            ...
            ${s.inputs.map(i=>`<input name="${i.name}" type="${i.type}" ${i.type!=="checkbox"?'style="width:6rem" required':""} value="${i.value}" ${i.min?`min="${i.min}"`:""} ${i.max?`max="${i.max}"`:""} ${i.checked?"checked":""}>`).join(" - ")}
          </div>
