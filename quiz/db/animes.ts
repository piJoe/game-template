...
export async function getRandomAnimesWithOpenings(
  ...
  if (options.excludeOpenings) {
    wheres.push(
      `anime_openings.id NOT IN (${options.excludeOpenings
        .map((_) => "?")
        .join(",")})`
    );
    queryParams.push(...options.excludeOpenings);
  }
  if (options.maxPopularity) {
    wheres.push(`animes.popularity <= ?`);
    queryParams.push(options.maxPopularity);
  ...
  return result.map((r) => {
    return {
      ...r,
      alternative_titles: JSON.parse(r.alternative_titles),
      openings: JSON.parse(r.openings),
    };
  });
}
